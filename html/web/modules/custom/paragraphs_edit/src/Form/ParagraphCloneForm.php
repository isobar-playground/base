...
class ParagraphCloneForm extends ContentEntityForm {
  ...
  public function form(array $form, FormStateInterface $form_state) {
    ...
    $bundles = [];
    $entity_type = $form_state->getValue(['paragraphs_edit', 'entity_type'], NULL) ?? $this->getFromUserInput($form_state, 'entity_type');
    ...
    $selection_settings = [];
    $bundle = $form_state->getValue(['paragraphs_edit', 'bundle'], NULL) ?? $this->getFromUserInput($form_state, 'bundle');
    if (!empty($bundle)) {
  ...
  public function validateForm(array &$form, FormStateInterface $form_state) {
    $destination_entity_type = $form_state->getValue(['paragraphs_edit', 'entity_type']) ?? $this->getFromUserInput($form_state, 'entity_type');
    $destination_entity_id = $form_state->getValue(['paragraphs_edit', 'parent']) ?? $this->getFromUserInput($form_state, 'parent');
    $destination_field = $form_state->getValue(['paragraphs_edit', 'field']) ?? $this->getFromUserInput($form_state, 'field');
  ...
  /**
   * Restore lost user input data.
   *
   * During cloning process, if user tries for example to upload image using
   * media library current form will is rebuilt and current context is lost.
   *
   ...
   *
   * @return mixed|null
   *   User provided value if exists, or NULL.
   */
  protected function getFromUserInput(FormStateInterface $form_state, $name) {
    if (!empty($form_state->getUserInput()['paragraphs_edit'][$name])) {
      return $form_state->getUserInput()['paragraphs_edit'][$name];
    }
